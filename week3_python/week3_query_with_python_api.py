...
product_category_reviews = reviews.select(reviews['product_category'])
product_category_reviews.show(n=50)
...
Most_helpful_review = reviews.select(reviews['product_title'],reviews['helpful_votes']).orderBy(reviews['helpful_votes'].desc())
Most_helpful_review.show(n=1)
#Question 7: How many reviews have a 5 star rating?
five_star_rating = reviews.where(reviews['star_rating'] == '5').count()
...
Reviews_with_integer_field = reviews.select(reviews['star_rating'].cast('int'), reviews['helpful_votes'].cast('int'), reviews['total_votes'].cast('int'))
Reviews_with_integer_field.show(n=10)
#Question 8: Find the date with the most purchases.
#Print the date and total count of the date with the most purchases
Total_Purchase_count = reviews.groupBy(reviews['purchase_date']).count().orderBy(desc('count'))
Total_Purchase_count.show(n=1)
...
reviews_with_timestamp.write.mode("overwrite").parquet("s3a://hwe-fall-2024/irajasekaran/bronze/reviews_static")
print("done")
...
customer_reviews = spark.read.csv("resources/customers.tsv.gz", sep="\t", header=True)
customer_reviews.write.mode("overwrite").parquet("s3a://hwe-fall-2024/irajasekaran/bronze/customers")
