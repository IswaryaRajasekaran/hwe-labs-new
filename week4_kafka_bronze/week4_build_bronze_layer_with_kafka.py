...
review_timestamp_df = df_new.withColumn("review_timestamp",current_timestamp())
...
#review_timestamp_df.write.mode("overwrite").parquet("s3a://hwe-fall-2024/irajasekaran/bronze/reviews")

query = review_timestamp_df \
.writeStream \
.outputMode("append") \
.format("parquet") \
.option("path", f's3a://hwe-fall-2024/irajasekaran/bronze/reviews') \
.option("checkpointLocation","c:/tmp/kafka-checkpoint") \
.start()
